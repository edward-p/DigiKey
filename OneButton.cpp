...
void OneButton::tick(void)
{
	...
	if (_state == 0) { // waiting for menu pin being pressed.
		...
		if ((buttonLevel == _buttonPressed) && ((unsigned long)(now - _stopTime) < _debounceTicks)) {
			// button was not released, stay at state 3
			_state = 3;
		...
		if (_TripleClickFunc == NULL || (unsigned long)(now - _stopTime) > _clickTicks) {
		...
		if (buttonLevel == _buttonReleased && ((unsigned long)(now - _stopTime) > _debounceTicks)) {
